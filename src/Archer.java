...
public class Archer extends Piece {
    ...
    public List<Square> getTargets(Board board) {
        ...
        // Northwest
        if (pos % 10 > 1 && pos >= 20 && b[pos-22].hasPiece() && (!b[pos-11].hasPiece()) && this.canCapture(b[pos-22]) && !(b[pos-22].getPiece() instanceof Pawn)) targets.add(b[pos-22]);
        // Northeast
        if (pos % 10 < 8 && pos >= 20 && b[pos-18].hasPiece() && (!b[pos-9].hasPiece()) && this.canCapture(b[pos-18]) && !(b[pos-18].getPiece() instanceof Pawn)) targets.add(b[pos-18]);
        // Southwest
        if (pos % 10 > 1 && pos < 80 && b[pos+18].hasPiece() && (!b[pos+9].hasPiece()) && this.canCapture(b[pos+18]) && !(b[pos+18].getPiece() instanceof Pawn)) targets.add(b[pos+18]);
        // Southeast
        if (pos % 10 < 8 && pos < 80 && b[pos+22].hasPiece() && (!b[pos+11].hasPiece()) && this.canCapture(b[pos+22]) && !(b[pos+22].getPiece() instanceof Pawn)) targets.add(b[pos+22]);
