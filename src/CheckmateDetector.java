...
public class CheckmateDetector {
    ...
    private boolean canBlock(List<Piece> threats,
        ...
        if (threats.size() == 1) {
            ...
            if (ks.getXCoordinate() == ts.getXCoordinate()) {
                int max = Math.max(ks.getYCoordinate(), ts.getYCoordinate());
                int min = Math.min(ks.getYCoordinate(), ts.getYCoordinate());

                for (int i = min + 1; i < max; i++) {
                    List<Piece> blks =
                            blockMoves.get(brdArray[i][ks.getXCoordinate()]);
                    ConcurrentLinkedDeque<Piece> blockers =
                            new ConcurrentLinkedDeque<Piece>();
                    blockers.addAll(blks);

                    if (!blockers.isEmpty()) {
                        movableSquares.add(brdArray[i][ks.getXCoordinate()]);

                        for (Piece p : blockers) {
                            if (testMove(p,brdArray[i][ks.getXCoordinate()])) {
                                blockable = true;
                            }
                        }
            ...
            if (ks.getYCoordinate() == ts.getYCoordinate()) {
                int max = Math.max(ks.getXCoordinate(), ts.getXCoordinate());
                int min = Math.min(ks.getXCoordinate(), ts.getXCoordinate());

                for (int i = min + 1; i < max; i++) {
                    List<Piece> blks =
                            blockMoves.get(brdArray[ks.getYCoordinate()][i]);
                    ConcurrentLinkedDeque<Piece> blockers =
                            new ConcurrentLinkedDeque<Piece>();
                    blockers.addAll(blks);
                    ...
                    if (!blockers.isEmpty()) {
                        ...
                        movableSquares.add(brdArray[ks.getYCoordinate()][i]);

                        for (Piece p : blockers) {
                            if (testMove(p, brdArray[ks.getYCoordinate()][i])) {
                                blockable = true;
                            }
                        }
            ...
            if (tC.equals(Queen.class) || tC.equals(Bishop.class)) {
                int kX = ks.getXCoordinate();
                int kY = ks.getYCoordinate();
                int tX = ts.getXCoordinate();
                int tY = ts.getYCoordinate();
    ...
    public boolean testMove(Piece p, Square sq) {
        ...
        if (!p.white() && blackInCheck()) movetest = false;
        else if (p.white() && whiteInCheck()) movetest = false;

        p.move(init);
        if (c != null) sq.placePiece(c);
